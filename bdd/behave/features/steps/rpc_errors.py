...
@then(u'It fails, telling me I must log in')
def step_impl(context):
  assert context.stubs.call_exc
  assert context.stubs.call_exc.code() == grpc.StatusCode.UNAUTHENTICATED
  assert context.stubs.call_exc.details().startswith("Must be authenticated to "), "Got error: " + context.stubs.call_exc.details()
